# NoShowApproval — reference (read on demand)
...
- The live `workflows/NoShowApproval.workflow` no longer calls `PATCH /xsodata/NoShowService.xsodata/NoShowApproval(...)` (destination `HANA_DB_DEV_LMS`) and no longer posts HTML to `employee/notification`; both survive only in `workflows/NoShowApproval-backup.workflow`, which is kept but not deployed. All three mail tasks of the live definition post data-only payloads to CPI `lms/sendDecisionMail`.
- `prepareInitiatorMailFormat.js`, `prepareInitialRequest.js` and `checkLeaveRequest.js` are **not** referenced by the live definition (`prepareInitialRequest.js` is referenced by the backup only).
- `AfterApprovalScript.js` sets `NO_SHOW_APPROVED` / `NO_SHOW_REJECTED` (with `cancellationReasonID: "NO SHOW"`), not `ENROLL-NO-SHOW-*`; `prepareInitialRequest.js` sets `ENROLL_NO_SHOW_In_Progress`.
